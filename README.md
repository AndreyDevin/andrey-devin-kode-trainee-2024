...
6.1
Прежде чем переходить к пункту 7, опять изменил UiState.
Сделал, что это не только sealed class состояния данных,
но и всегда несет в себе data class с выбором который сделал пользователь
(департамент, текст в поисковой строке, тип сортировки)
Чтобы при смене состояния экрана, всегда можно было восстановить состояние UI.
И всё это, вместе с данными от API, содержится в одном объекте по принципу единого источника данных.
Заняло это 80 мин.
